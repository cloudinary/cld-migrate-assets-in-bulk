...
//
const scriptLog = logging.script;
const payloadLog = logging.payload;
...
async function loopOverCsvInput_Async (cliArgs, cliCommand, payloadModule, confirmationRoutinesModule) {
    ...
    await async.mapLimit(inputRecordGeneratorAsync, maxConcurrentUploads, async (input) => {
        ...
        try {
            stats.concurrent += 1;
            stats.attempted += 1;
            ({ payload, plugins_trace } = input2ApiPayload(input));
            ...
            payloadLog.info({input, payload, response, summary, plugins_trace});
