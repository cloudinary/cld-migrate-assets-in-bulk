...
async function loadPlugins_Async(pluginsFolderPath) {
    ...
    for (const pluginFilePath of resolvedPluginFiles) {
        ...
        try {
            ...
            const moduleLogger = logging.createPluginLogger(pluginName);
            await module.plugin.init_Async(moduleLogger);
...
/**
 * Produces array of the plugin names for successfully activated plugins
 * (those registered in _LOADED_PLUGINS)
 * 
 * @returns {Array} List of successfully activated plugin names
 */
function listLoadedPlugins() {
    return Object.keys(_LOADED_PLUGINS);
}
